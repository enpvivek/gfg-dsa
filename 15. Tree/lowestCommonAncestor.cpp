...
int diameterOfBinaryTree(Node *root)
{
    ...
    int d1 = 1 + heightOfTree(root->left) + heightOfTree(root->right);
...
Node *lowestCommonAncestors(Node *root, int x, int y)
{
    if (root == NULL)
        return NULL;
    vector<Node *> pathX;
    vector<Node *> pathY;
    if (findPath(root, pathX, x) == false || findPath(root, pathY, y) == false)
    {
        return NULL;
    }
    for (int i = 0; i < pathX.size() - 1 && i < pathY.size() - 1; i++)
    {
...
int main()
{
    ...
    cout << res << endl;
    cout << lowestCommonAncestors(root, 40, 50)->key << endl;
