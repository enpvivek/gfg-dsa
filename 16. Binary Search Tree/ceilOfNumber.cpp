...
Node *findCeil(Node *root, int x, Node *closestSoFar = NULL)
{
    if (root == NULL)
        return closestSoFar;
    if (root->key > x)
    ...
    closestSoFar = findCeil(root->left, x, closestSoFar);
    closestSoFar = findCeil(root->right, x, closestSoFar);
    return closestSoFar;
}
...
int main()
{
    ...
    cout << findCeil(root, 41)->key << endl;
